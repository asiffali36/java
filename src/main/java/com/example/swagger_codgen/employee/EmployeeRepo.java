...
public class EmployeeRepo {
    ...
    public ResponseEntity<Object> insertEmployeeRecord(Employee modelClass) {
        String query = "insert into employee_data(id,firstname,lastname) values(?,?,?)";

       int update= this.jdbcTemplate.update(query,modelClass.getId(),modelClass.getFirstName(), modelClass.getLastName());
       if (update==1) {
    ...
    public int updateRecord(Employee modelClass,int id) {
        String query = "UPDATE employee_data\n" +
                "SET firstname = ?, lastname= ?" +
                "WHERE id = ?";
        return this.jdbcTemplate.update(query, modelClass.getFirstName(), modelClass.getLastName(),id);
    }
}
